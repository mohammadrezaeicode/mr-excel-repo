...
export function ExcelTable(data: ExcelTable) {
  ...
  let styleGenerator = styleKeys.reduce(
    (res, cur, index) => {
...
<si>
    ...
    `;
      ...
      sheetDataString = `<row r="${rowCount}" spans="1:${colsLength}"  ${
      ...
      if (Array.isArray(sheetData.data)) {
        ...
        sheetData.data.forEach((mData, innerIndex) => {
          ...
          sheetDataString += `<row r="${rowCount}" spans="1:${colsLength}" ${
            ...
            keyOutline in mData ? `outlineLevel="${mData[keyOutline]}"` : ""
          ...
          objKey.forEach((key, keyIndex) => {
            ...
            if (typeof dataEl != "undefined") {
              if (typeof dataEl == "string") {
                sheetDataString += `
<c r="${cols[keyIndex]}${rowCount}" t="s" ${
...
<c r="${cc}${cr}" ${
    ...
    workbookString += `<sheet state="${shState}" name="${shName}" sheetId="${
      index + 1
    }" r:id="rId${indexId + 1}" />`;
    ...
    mapData["sheet" + (index + 1)] = {
      ...
      sheetDataString,
      sheetSizeString:
        sheetSizeString.length > 0 ? `<cols>${sheetSizeString}</cols>` : "",
...
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="${
      sharedStringIndex - 1
    }" uniqueCount="${Object.keys(sharedStringMap).length}">
    ${sharedString}
</sst>`
  );
