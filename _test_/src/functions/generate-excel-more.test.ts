...
describe("generateExcel additional branch coverage", () => {
  test("styles key with undefined value covers optional chaining nullish branch", async () => {
  ...
  test("throws when modified is invalid date string", async () => {
    const excelTable: ExcelTable<{ a: string }> = {
      notSave: true,
      modified: "not-a-date",
      sheet: [{ headers: [], data: [] }],
