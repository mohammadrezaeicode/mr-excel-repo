...
function ExcelTable(data) {
    ...
    let styleGenerator = styleKeys.reduce((res, cur, index) => {
        ...
        if (styl.fg) {
            ...
            <patternFill patternType="solid">
                ${styl.fg ? `<fgColor rgb="${styl.fg.replace("#", "")}" />` : ""}
            </patternFill>
        </fill>`;
        }
        ...
        res.cell.value =
            res.cell.value +
                `<xf numFmtId="0" fontId="${indexes.fontIndex}" fillId="${indexes.fillIndex}" borderId="0" xfId="0" applyFill="${indexes.applyFill}" />`;
    ...
    // styleKeys.reduce((res, cur, index) => {
    //     const styl = data.styles[cur]
    //     console.log('test', cur)
    //     if (styl.bg || styl.fg) {
    //         data.styles[cur].index = index + 2
    //         console.log(data.styles)
    //         return res + `<xf numFmtId="0" fontId="0" fillId="${index + 2}" borderId="0" xfId="0" applyFill="1" />`;
    //     } else {
    //         return res
    //     }
    // }, "")
    xlFolder === null || xlFolder === void 0 ? void 0 : xlFolder.file("styles.xml", `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
    xmlns:x14ac="http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac"
    xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006">
    <fonts count="${styleGenerator.font.count}">
         <font>
            <sz val="11" />
            <color theme="1" />
            <name val="Calibri" />
        ...
        ${styleGenerator.font.value}
    </fonts>
    <fills count="${styleGenerator.fill.count}">
        <fill>
    ...
    <cellXfs count="${styleGenerator.cell.count}">
    ...
    for (let index = 0; index < sheetLength; index++) {
        ...
        if (Array.isArray(sheetData.headers) && sheetData.headers.length) {
            sheetData.headers.forEach((v, innerIndex) => {
                if (v.size && v.size > 0) {
                    sheetSizeString += `<col min="${innerIndex}" max="${innerIndex}" width="${v.size}" customWidth="1" />`;
                }
                sheetDataString += `
<c r="${cols[innerIndex]}${rowCount}" t="s">
...
<si>
    ...
    `;
            ...
            sheetDataString = `<row r="${rowCount}" spans="1:${colsLength}"  ${sheetData.headerHeight
                ? `ht="${sheetData.headerHeight}" customHeight="1"`
                : ""} ${sheetData.headerRowOption
                ? Object.keys(sheetData.headerRowOption).reduce((res, curr) => {
                    return (res + " " + curr + '="' + sheetData.headerRowOption[curr] + '" ');
                }, "  ")
                : ""}>
${sheetDataString}
</row>
            ...
            if (Array.isArray(sheetData.data)) {
                let hasFurmul = (_a = data.formula) === null || _a === void 0 ? void 0 : _a.useFormula;
                let cc = "";
                let cr = -1;
                ...
                sheetData.data.forEach((mData, innerIndex) => {
                    ...
                    sheetDataString += `<row r="${rowCount}" spans="1:${colsLength}" ${keyHeight in mData
                        ? `ht="${mData[keyHeight]}" customHeight="1"`
                        : ""}  ${keyOutline in mData ? `outlineLevel="${mData[keyOutline]}"` : ""} ${keyHidden in mData ? `hidden="${mData[keyHidden]}"` : ""}>`;
                    objKey.forEach((key, keyIndex) => {
                        const dataEl = mData[key];
                        if (typeof dataEl != "undefined") {
                            if (typeof dataEl == "string") {
                                sheetDataString += `
<c r="${cols[keyIndex]}${rowCount}" t="s" ${rowStyle ? `s="${data.styles[rowStyle].index}"` : ""}>
    <v>${sharedStringIndex}</v>
</c>
                        `;
                                sharedString += `
<si>
    ...
    `;
                                ...
                                sheetDataString += `
<c r="${cols[keyIndex]}${rowCount}" ${rowStyle ? `s="${data.styles[rowStyle].index}"` : ""}>
    <v>${dataEl}</v>
</c>
                        `;
                            }
                    ...
                    if (cr == rowCount) {
                        ...
                        sheetDataString += `
<c r="${cc}${rowCount}">
    <f>${(_a = data.formula) === null || _a === void 0 ? void 0 : _a.type}(${(_b = data.formula) === null || _b === void 0 ? void 0 : _b.start}:${(_c = data.formula) === null || _c === void 0 ? void 0 : _c.end})</f>
</c>
                        `;
                    }
                    rowCount++;
                ...
                if (data.formula && hasFurmul) {
                    sheetDataString += `<row r="${cr}"  ${"height" in data.formula
                        ? `ht="${data.formula.height}" customHeight="1"`
                        : ""}>
<c r="${cc}${cr}" ${"styleId" in data.formula
                        ? `s="${data.styles[data.formula.styleId].index}"`
                        : ""}>
    <f>${data.formula.type}(${data.formula.start}:${data.formula.end})</f>
</c>
</row>
                        `;
                }
        ...
        workbookString += `<sheet state="${shState}" name="${shName}" sheetId="${index + 1}" r:id="rId${indexId + 1}" />`;
        workbookRelString += `<Relationship Id="rId${indexId + 1}"
        Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet"
        Target="worksheets/sheet${index + 1}.xml" />`;
        sheetNameApp += `
        <vt:lpstr>${"sheet" + (index + 1)}</vt:lpstr>
        `;
        ...
        mapData["sheet" + (index + 1)] = {
            ...
            sheetName: shName,
            sheetDataString,
            sheetSizeString: sheetSizeString.length > 0 ? `<cols>${sheetSizeString}</cols>` : "",
            protectionOption: sheetData.protectionOption
                ? Object.keys(sheetData.protectionOption).reduce((res, cu) => {
                    return (res +
                        ` ${cu}="${sheetData.protectionOption[cu]}" `);
                }, "<sheetProtection") + "/>"
                : "",
        ...
        <sheetView workbookViewId="0" />
            ...
            tabColor: sheetData.tabColor
                ? `<sheetPr codeName="${"Sheet" + (index + 1)}">
        <tabColor rgb="${sheetData.tabColor.replace("#", "")}" />
    </sheetPr>`
                : `<sheetPr>
...
<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types">
    ...
    ${sheetContentType}
    ${((_b = data.formula) === null || _b === void 0 ? void 0 : _b.useFormula) && false
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:creator>${data.creator}</dc:creator><cp:lastModifiedBy>${data.lastModifiedBy}</cp:lastModifiedBy><dcterms:created xsi:type="dcterms:W3CDTF">${data.created}</dcterms:created><dcterms:modified xsi:type="dcterms:W3CDTF">${data.modified}</dcterms:modified></cp:coreProperties>`);
    docPropsFolder === null || docPropsFolder === void 0 ? void 0 : docPropsFolder.file("app.xml", `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties"
    ...
    <HeadingPairs>
        <vt:vector size="2" baseType="variant">
            ...
            <vt:variant>
                <vt:i4>${sheetLength}</vt:i4>
            </vt:variant>
        </vt:vector>
    </HeadingPairs>
    <TitlesOfParts>
        <vt:vector size="${sheetLength}" baseType="lpstr">
            ${sheetNameApp}
        </vt:vector>
    </TitlesOfParts>
    <Company></Company>
...
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="${sharedStringIndex - 1}" uniqueCount="${Object.keys(sharedStringMap).length}">
    ${sharedString}
</sst>`);
    //xl/_rels
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
    ...
    ${workbookRelString}
    ${((_c = data.formula) === null || _c === void 0 ? void 0 : _c.useFormula) && false
        ? `<Relationship Id="rId${indexId}"
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
    ...
    zip.generateAsync({ type: "blob" }).then(function (content) {
        // see FileSaver.js
        (0, file_saver_1.saveAs)(content, "Excel_File.xlsx");
    });
}
